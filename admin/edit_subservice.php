...
<div class="col-lg-6" style=" width: 100%;">
 <?php
                                    if(isset($_GET['id']))
                                    {
                                        $row=$subservice_model->get_subservice($_GET['id']);
                                        //$row=$service_model->stripslahes_rows($row);
                                    ?>
    <form role="form" id="form" action="controller/subservice_controller.php" method="post" enctype="multipart/form-data">
...
{
    ?>
    <img src="../upload/subservice/download.png" height="100" width="100" alt=""/>
     <?php                                           
}
 else {
...
?>
    <img src="../upload/subservice/<?php echo $row['image']?>" height="100" width="100" alt=""/>
<?php
 }
?>
</div>
...
<input name="name" type="text" id="name" class="form-control validate[required]" placeholder="service" value="<?php echo $row['name'];?>">
</div>
...
<label class="radio-inline">
    ...
    <div class="form-group">
                                            ...
                                            <select id="order" name="num" class="form-control validate[required]" >
                                                <option value="">select</option>
                                                <?php
                                                $service_model->order_selected($row['num']);
                                                ?>
                                            </select>
                                        </div> 
     <div class="form-group">
<label> Service Icon  (should be 550*165 px ):</label>
 <input type="file" name="file[]" class=""  id="img"/>
</div>
    
<input id="formbutton" name="editsubservice" type="submit" value="Submit" class="btn btn-primary"/>
